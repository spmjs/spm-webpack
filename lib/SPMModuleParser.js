...
function SPMModuleParser(cwd, request, callback) {
  ...
  if (request.path.indexOf('node-libs-browser') > -1) {
    var _filepath = nodeResolve.sync(request.request, {basedir:request.path});
  ...
  var expectVersion;
  if (currPkg && currPkg.spm && ((currPkg.spm.dependencies && currPkg.spm.dependencies[module.name]) || (currPkg.spm.devDependencies && currPkg.spm.devDependencies[module.name]))) {
    if (currPkg.spm.dependencies) expectVersion = currPkg.spm.dependencies[module.name];
    else expectVersion = currPkg.spm.devDependencies[module.name];
