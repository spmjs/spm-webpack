...
function SPMModuleParser(cwd, request, callback) {
  ...
  if (currPkg && currPkg.name === module.name) {
    var filepath = join(currPkgPath, module.file || currPkg.spm.main || 'index.js');
  ...
  var expectVersion;
  if (isRoot && !currPkg || !currPkg.spm) {
    expectVersion = '*';
  } else if (currPkg && currPkg.spm && currPkg.spm.dependencies && currPkg.spm.dependencies[module.name]) {
  ...
  // TODO: file 可能需要补充后缀
  var filepath = join(module.path, module.file || module.pkg.spm.main || 'index.js');
  log.info('filepath', filepath);
