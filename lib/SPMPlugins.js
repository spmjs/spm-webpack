...
SPMPlugins.prototype.apply = function(compiler) {
  ...
  compiler.plugin('done', function(stats) {
    ...
    function copy(files) {
      copyFiles(files, opts.outputPath, opts.cwd, compiler.outputFileSystem);
    }
...
function copyFiles(files, destDir, cwd, fs) {
  for (var k in files) {
    copyFile(k, files[k], destDir, cwd, fs);
  }
}

function copyFile(file, destFile, destDir, cwd, fs) {
  var _fs = require('fs');
  ...
  (fs.mkdirpSync || require('mkdirp'))(dirname(target));
  var err = fs.writeFileSync(target, _fs.readFileSync(ori));
  if (err) {
    log.error('copy', err);
  } else {
