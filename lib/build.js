...
function getWebpackOpts(opts, callback) {
  ...
  install({
    ...
    if (opts.build.vendor) {
      ...
      args.plugins.push(new webpack.optimize.CommonsChunkPlugin('vendor', utils.getPrefix(pkg)+'vendor.bundle.js'));
    }

    if (opts.build.common) {
      args.plugins.push(new webpack.optimize.CommonsChunkPlugin(utils.getPrefix(pkg)+'common.js'));
    }
