...
module.exports = function build(opts, callback) {
  installDeps(opts, function() {
    ...
    compiler.plugin('done', function(stats) {
      ...
      log.info('build', 'done');
      if (Object.keys(webpackOpts.files.other).length) copyFiles(webpackOpts.files.other, webpackOpts.output.path, opts.cwd);
...
function copyFiles(files, destDir, cwd) {
  for (var k in files) {
    var destFile = join(destDir, k);
    var err = copy(join(cwd, files[k]), destFile);
    if (err) {
      log.error('copy', err);
    } else {
      log.info('copy', k);
